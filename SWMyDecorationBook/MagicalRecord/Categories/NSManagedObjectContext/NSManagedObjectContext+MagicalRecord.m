...
static NSManagedObjectContext *NXMagicalRecordMyVaultFileContext;
static NSManagedObjectContext *NXMagicalRecordFavoriteFileContext;
static NSManagedObjectContext *NXMagicalRecordRepoFileContext;
static NSManagedObjectContext *NXMagicalRecordBackgroundContext;
static NSManagedObjectContext *NXMagicalRecordCacheFileContext;
...
+ (void) MR_initializeDefaultContextWithCoordinator:(NSPersistentStoreCoordinator *)coordinator;
{
    ...
    if (MagicalRecordDefaultContext == nil)
    {
        ...
        /**=========== custome moc for skyDRM================ */
        
        NSManagedObjectContext *backContext = [self MR_contextWithParent:defaultContext];
        [self MR_setBackgroundContext:backContext];
        
//        NSManagedObjectContext *myVaultFileContext = [self MR_contextWithParent:defaultContext];
//        [self MR_setMyVaultFileContext:myVaultFileContext];
//        
//        NSManagedObjectContext *favFileContext = [self MR_contextWithParent:defaultContext];
//        [self MR_setFavoriteFileContext:favFileContext];
//        
//        NSManagedObjectContext *repoFileContext = [self MR_contextWithParent:defaultContext];
//        [self MR_setRepoFileContext:repoFileContext];
//        
//        NSManagedObjectContext *cacheFileContext = [self MR_contextWithParent:defaultContext];
//        [self MR_setCacheFileContext:cacheFileContext];
    }
}
...
+ (MR_nonnull NSManagedObjectContext *) MR_myVaultFileContext
{
    @synchronized(self) {
        NSAssert(NXMagicalRecordMyVaultFileContext != nil, @"MyVaultFile context is nil! Did you forget to initialize the Core Data Stack?");
        return NXMagicalRecordMyVaultFileContext;
    }
}

+ (MR_nonnull NSManagedObjectContext *) MR_favoriteFileContext
{
    @synchronized(self) {
        NSAssert(NXMagicalRecordFavoriteFileContext != nil, @"favoriteFile context is nil! Did you forget to initialize the Core Data Stack?");
        return NXMagicalRecordFavoriteFileContext;
    }
}

+ (MR_nonnull NSManagedObjectContext *) MR_repoFileContext
{
    @synchronized(self) {
        NSAssert(NXMagicalRecordRepoFileContext != nil, @"repoFile context is nil! Did you forget to initialize the Core Data Stack?");
        return NXMagicalRecordRepoFileContext;
    }
}

+ (MR_nonnull NSManagedObjectContext *) MR_backgroundContext
{
    @synchronized(self) {
        NSAssert(NXMagicalRecordBackgroundContext != nil, @"Background context is nil! Did you forget to initialize the Core Data Stack?");
        return NXMagicalRecordBackgroundContext;
    }
}

+ (MR_nonnull NSManagedObjectContext *) MR_cacheFileContext
{
    @synchronized(self) {
        NSAssert(NXMagicalRecordCacheFileContext != nil, @"cacheFile context is nil! Did you forget to initialize the Core Data Stack?");
        return NXMagicalRecordCacheFileContext;
    }
}
...
+ (void)rootContextDidSaveForBackgoundContext:(NSNotification *)notification
{
    if ([notification object] != [self MR_rootSavingContext])
    {
        return;
    ...
    [[self MR_backgroundContext] performBlock:^{
        for (NSManagedObject *object in [[notification userInfo] objectForKey:NSUpdatedObjectsKey])
        {
            [[[self MR_backgroundContext] objectWithID:[object objectID]] willAccessValueForKey:nil];
        }
        [[self MR_backgroundContext] mergeChangesFromContextDidSaveNotification:notification];
    }];
...
+ (void) MR_cleanUp
{
    ...
    [self MR_setBackgroundContext:nil];
...
+ (void) MR_setMyVaultFileContext:(NSManagedObjectContext *)moc
{
    NXMagicalRecordMyVaultFileContext = moc;
}

+ (void)MR_setFavoriteFileContext:(NSManagedObjectContext *)moc
{
    NXMagicalRecordFavoriteFileContext = moc;
}

+ (void)MR_setRepoFileContext:(NSManagedObjectContext *)moc
{
    NXMagicalRecordRepoFileContext = moc;
}

+ (void) MR_setBackgroundContext:(NSManagedObjectContext *)moc
{
    if (NXMagicalRecordBackgroundContext) {
        [[NSNotificationCenter defaultCenter] removeObserver:NXMagicalRecordBackgroundContext];
    }
    NXMagicalRecordBackgroundContext = moc;
    if ((NXMagicalRecordBackgroundContext != nil) && ([self MR_rootSavingContext] != nil)) {
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(rootContextDidSaveForBackgoundContext:)
                                                     name:NSManagedObjectContextDidSaveNotification
                                                   object:[self MR_rootSavingContext]];
    }
}

+ (void)MR_setCacheFileContext:(NSManagedObjectContext *)moc
{
    NXMagicalRecordCacheFileContext = moc;
}
